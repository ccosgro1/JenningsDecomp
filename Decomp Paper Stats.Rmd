...
###NOT SIGNIFICANT
```
```{r decompK lme~TerrType}
####TO DO####
# lme of decompk compared to TerrType
decompkterr=merge(decompk.allspec.col,plotdesig)
colnames(decompkterr)
rownames(decompkterr)=c(1:63)
carovaterr=lme(decompk.carova~TerrType,random=~1|Plot, data=decompkterr)
anova(carovaterr)
summary(carovaterr)
...
dim(hydroact.dat) #we did a fuck ton of hydrolase enzymes, thus all the rows. Dont't freak out about mix-matched data from the merge
